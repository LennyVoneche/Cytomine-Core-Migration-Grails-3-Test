...
//        def consumer = grailsApplication.config.grails.LTIConsumer.find{it.key == params.oauth_consumer_key}
        def consumer = Metadata.current.'grails.LTIConsumer'.toString().find{it.key == params.oauth_consumer_key}
        log.info consumer
...
//            OAuthMessage oam = OAuthServlet.getMessage(request, grailsApplication.config.grails.serverURL+request.forwardURI);
            OAuthMessage oam = OAuthServlet.getMessage(request,Metadata.current.'grails.serverURL'.toString()+request.forwardURI);
