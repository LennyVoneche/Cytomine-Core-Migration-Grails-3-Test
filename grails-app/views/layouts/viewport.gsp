...
<%@ page import="grails.util.Environment; grails.core.GrailsApplication" %>
...
<g:if test="${Environment.getCurrent() == Environment.DEVELOPMENT || Environment.getCurrent() == Environment.CUSTOM }">
    ...
    <script type="text/javascript">
        $(function() {
            ...
            window.app.coreServer =  "${Metadata.current.'grails.serverURL'.toString()}";
            %{--window.app.uploadServer =  "${grailsApplication.config.grails.uploadURL}";--}%
            window.app.uploadServer =  "${Metadata.current.'grails.uploadURL'.toString()}";
            window.app.params =  {};
            window.app.params.readOnlyProjectsByDefault =  "${Metadata.current.'grails.readOnlyProjectsByDefault'.toString()}";
        });
    </script>
</g:if>

<g:if test="${Environment.getCurrent() == Environment.PRODUCTION}">
    <%print 'viewport.gsp 008'%>
