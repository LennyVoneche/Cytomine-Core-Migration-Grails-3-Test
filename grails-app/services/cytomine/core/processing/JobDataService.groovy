...
class JobDataService extends ModelService {
    ...
    def read(def id) {
        ...
        if(jobData) {
            securityACLService.check(jobData.container(), BasePermission.READ)
        }
        jobData
    }
     ...
     */
    def add(def json) {
        securityACLService.check(json.job, Job,"container", BasePermission.READ)
    ...
    def update(JobData jd, def jsonNewData) {
        securityACLService.check(jd.container(), BasePermission.READ)
