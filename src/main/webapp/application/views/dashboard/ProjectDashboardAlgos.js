...
var ProjectDashboardAlgos = Backbone.View.extend({
    ...
    buildJobPropertiesElem: function (job) {
        ...
        var data = [];
        new PropertyCollection({domainClassName:"cytomine.core.processing.Job", domainIdent:job.get('id')}).fetch({
            success: function (collection, response) {
    ...
    buildJobAttachedFilesElem: function (job) {
        ...
        var data = [];
        new AttachedFileCollection({domainClassName:"cytomine.core.processing.Job", domainIdent:job.get('id')}).fetch({
            success: function (collection, response) {
                collection.each(function (model) {
    ...
    createJobParameterDomainValue: function (ids, collection, param, maxSize) {
        var getLink = function(model, uriPrintAttribut) {
            if (model.get("class") == 'cytomine.core.project.Project') {
                return _.template("<a href='#tabs-dashboard-<%= id %>'><%= name %></a>", { id : model.id, name : model.get(uriPrintAttribut) });
            } else if (model.get("class") == 'cytomine.core.image.ImageInstance') {
            ...
            } else if (model.get("class") == 'cytomine.core.ontology.Term') {
